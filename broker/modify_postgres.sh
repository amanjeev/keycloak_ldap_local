...
docker exec postgresdbbroker ./tmp/setup_postgres.sh
wait

echo "Confirming schema alterations"
VERIFY=`docker exec postgresdbbroker psql -X -A -U keycloak -c "\d+ user_attribute" | grep -o "value|character varying|"`
